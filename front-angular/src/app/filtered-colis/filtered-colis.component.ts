...
@Component({
  selector: 'app-filtered-colis',
  templateUrl: './filtered-colis.component.html',
  styleUrls: ['./filtered-colis.component.css']
})
export class FilteredColisComponent implements OnInit {
  displayedColumns: string[] = ['select', 'Numero', 'barcode', 'creationDate', 'chargeur', 'livreA', 'de', 'a', 'type', 'frais', 'crbt', 'status', 'actions'];
  @Input() dataSource: MatTableDataSource<Colis> = new MatTableDataSource<Colis>([]);
  ...
  onPageChange(): void {
    this.calculateTotals();
    this.updateSelections(); // Update selections when page changes
  }
